...
func (runner *StartRunner) Run(c *cli.Context) error {
	...
	server := &server.GracefulShutdownServer{
		...
		RequestMembership:      handlers.NewRequestMembershipHandler(logger, mysqlDatabaseClient),
		UpdateGroupMembership:  handlers.NewUpdateMembershipHandler(logger, mysqlDatabaseClient),
		CheckPendingMembership: handlers.NewCheckPendingMembershipRequestsHandler(logger, mysqlDatabaseClient),
	}
	server.Start()
	return nil
}
